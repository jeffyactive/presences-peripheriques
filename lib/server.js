...
middleware.on('visibilityEvent', function(tiraid) {
  var strongestRssi = tiraid.radioDecodings[0].rssi;

  if(!isReelyActive(tiraid) && (strongestRssi > config.MIN_RSSI)) {
...
function convertToLightState(tiraid) {
  var state = [ { bri: 0, offDelay: config.HUE_PULSE_MILLISECONDS },
                { bri: 0, offDelay: config.HUE_PULSE_MILLISECONDS } ];
...
function setLightState(index, state) {
  ...
  var delayedOff = function(index, lightState) {
    var lightState = hue.lightState.create().transition(0);
    setTimeout(function() {
